...
Map<String, dynamic> encodeValue(dynamic value) {
  ...
  if (value is Map) {
    ...
    return {
      'mapValue': {'fields': value.map((k, v) => MapEntry(k, encodeValue(v)))},
    };
  }
...
Future<void> createDoc(
  ...
  final url = Uri.parse(
    'https://firestore.googleapis.com/v1/projects/sahaya-7df6d/databases/(default)/documents/$collectionId?documentId=$documentId',
  );
  ...
  if (response.statusCode >= 200 && response.statusCode < 300) {
    ...
    print(
      'Failed to insert $documentId: ${response.statusCode} - ${response.body}',
    );
...
void main() async {
  ...
  await createDoc('problem_cards', 'pc_ambattur', {
    ...
    'affectedCount': 500,
    'description':
        'Severe water pipe burst affecting 500 households, requires emergency tank distribution.',
  ...
  await createDoc('problem_cards', 'pc_kodambakkam', {
    ...
    'affectedCount': 50,
    'description':
        'Local corporation school requires volunteers to aid with high school enrollment processing.',
  ...
  await createDoc('problem_cards', 'pc_adyar', {
    ...
    'affectedCount': 100,
    'description':
        'Weekly community kitchen looking for resources and volunteers to serve 100 meals.',
