...
class _AiAssistantSheetState extends State<AiAssistantSheet> {
  ...
  GeoPoint? _asGeoPoint(dynamic value) {
    ...
    if (value is Map) {
      final rawLat = value['latitude'] ?? value['lat'];
      final rawLng = value['longitude'] ?? value['lng'] ?? value['lon'];
  ...
  dynamic _coerceToOriginalType(dynamic incoming, dynamic original) {
    if (original is GeoPoint) {
      return _asGeoPoint(incoming) ?? original;
    }
    if (original is int) {
  ...
  Map<String, dynamic> _safePreviewForApply() {
    ...
    for (final entry in preview.entries) {
      ...
      if (key.toLowerCase().contains('geopoint')) {
        final parsed = _asGeoPoint(entry.value);
        if (parsed != null) {
          out[key] = parsed;
        }
        continue;
