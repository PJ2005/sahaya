...
class NgoHomeScreen extends StatelessWidget {
            ...
            slivers: [
              ...
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      Text('ACTIVE PROBLEM CARDS', 
                        style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w800, color: Theme.of(context).colorScheme.onSurfaceVariant, letterSpacing: 1)),
                      const Spacer(),
              ...
              children: [
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Good morning,', style: GoogleFonts.inter(color: Colors.white60, fontSize: 13)),
                      Text('NGO Coordinator', style: GoogleFonts.inter(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w800)),
                    ],
                  ),
                ),
                CircleAvatar(
  ...
  Widget _statsItem(String label, String val) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(val, style: GoogleFonts.inter(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w800)),
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        return IconButton(
          ...
          onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ProofReviewScreen(ngoId: ngoId))),
        );
      },
    );
  }
  ...
  Widget _emptyState(BuildContext context) {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.dashboard_customize_outlined, size: 64, color: Theme.of(context).colorScheme.outlineVariant),
          const SizedBox(height: 16),
          Text('No approved cards yet', style: GoogleFonts.inter(fontWeight: FontWeight.w700, color: Theme.of(context).colorScheme.onSurfaceVariant)),
          const SizedBox(height: 8),
          Text('Approve cards from the Review Queue', style: GoogleFonts.inter(fontSize: 13, color: Theme.of(context).colorScheme.onSurfaceVariant)),
        ],
      ),
    );
  }
...
class NgoTaskBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    switch (card.severityLevel) {
      case SeverityLevel.critical: severityColor = SahayaColors.coral; break;
      case SeverityLevel.high: severityColor = SahayaColors.amber; break;
      case SeverityLevel.medium: severityColor = SahayaColors.emerald; break;
      case SeverityLevel.low: severityColor = Colors.teal; break;
    }

    return GestureDetector(
      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => NgoTaskDetailScreen(card: card))),
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: severityColor.withValues(alpha: 0.6), width: 2),
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                _miniPill(context, card.issueType.name.replaceAll('_', ' ').toUpperCase(), cs.primary.withValues(alpha: 0.1), cs.primary),
                const Spacer(),
                Text('PRIORITY ${card.priorityScore.toInt()}', 
                  style: GoogleFonts.inter(fontWeight: FontWeight.w900, color: severityColor, fontSize: 11, letterSpacing: 0.5)),
              ],
            ),
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 4),
                Text('${card.affectedCount}', style: GoogleFonts.inter(fontSize: 12, color: cs.onSurfaceVariant, fontWeight: FontWeight.w700)),
              ],
            ),
          ],
        ),
