...
""";
    ...
    try {
      ...
      for (var struct in jsonList) {
        ...
        IssueType iType = IssueType.values.firstWhere(
          (e) => e.name == issueStr,
          orElse: () => IssueType.other,
        );

        String severityStr =
            struct['severityLevel']?.toString().toLowerCase() ?? '';
        SeverityLevel sLevel = SeverityLevel.values.firstWhere(
        ...
        multiCards.add(
          ProblemCard(
            ...
            description:
                struct['description'] ??
                'Extracted organically without valid descriptions.',
            confidenceScore:
                (struct['confidenceScore'] as num?)?.toDouble() ?? 0.0,
